...
namespace SO {
	...
	void GameModel::createVertexBuffers(const std::vector<Vertex>& vertices, Model& out, RendererDevice& rDevice) {
		out.vertexCount = static_cast<uint32_t>(vertices.size());
		assert(out.vertexCount >= 3 && "Vertex Count must be greater or equale to 3!");
		VkDeviceSize bufferSize = sizeof(vertices[0]) * out.vertexCount;
		uint32_t vertexSize = sizeof(vertices[0]);
	...
	void GameModel::createIndexBuffers(const std::vector<uint32_t>& indices, Model& out, RendererDevice& rDevice) {
		out.indexCount = static_cast<uint32_t>(indices.size());
		assert(out.indexCount >= 3 && "Index Count must be greater or equale to 3!");
		VkDeviceSize bufferSize = sizeof(indices[0]) * out.indexCount;
		uint32_t indexSize = sizeof(indices[0]);
	...
	void GameModel::loadModel(const std::string& path, Model& m, RendererDevice& rDevice) {
		tinyobj::attrib_t attrib;
		std::vector<tinyobj::shape_t> shapes;
		std::vector<tinyobj::material_t> materials;
		std::string warn, err;
		...
		m.vertexCount = vertices.size();
		createVertexBuffers(vertices, m, rDevice);
		m.indexCount = indices.size();
		if (m.indexCount > 0) {
			m.hasIndexBuffer = true;
			createIndexBuffers(indices, m, rDevice);
		}
		else { m.hasIndexBuffer = false; }
	}
	...
	Model GameModel::loadModel(const std::string& path, RendererDevice& device) {
		std::shared_ptr<Model> m = std::make_shared<Model>();
		loadModel(path, *m, device);
		return *m.get();
	}
