#include "debug_collision_render_system.hpp"

#include "Components/Physics/box_collider.hpp"
...
namespace Systems {
	...
	void DebugCollisionRenderSystem::init(std::shared_ptr<SO::RendererDevice>& device, VkRenderPass renderPass, VkDescriptorSetLayout descriptorSetLayout)
	{
		rDevice = device;
	...
	void DebugCollisionRenderSystem::shutDown() {
		vkDestroyPipelineLayout(rDevice->device(), pipelineLayout, nullptr);
	}


	void DebugCollisionRenderSystem::createPipelineLayout(VkDescriptorSetLayout descriptorSetLayout) {
	...
	void DebugCollisionRenderSystem::createPipeline(VkRenderPass renderPass) {
		...
		gPipeline = std::make_unique<SO::Pipeline>(*rDevice, "C:/VStudio/StartOver/StartOver/shaders/out/debug_coll.vert.spv", "C:/VStudio/StartOver/StartOver/shaders/out/debug_coll.frag.spv", configInfo);
	}

	void DebugCollisionRenderSystem::bind(VkCommandBuffer commandBuffer, SO::Model& m) {
	...
	void DebugCollisionRenderSystem::draw(VkCommandBuffer commandBuffer, SO::Model& m) {
		if (m.hasIndexBuffer) {
	...
	void DebugCollisionRenderSystem::render(SO::FrameInfo& frameInfo) {
		gPipeline->bind(frameInfo.commandBuffer);

		vkCmdBindDescriptorSets(frameInfo.commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 1, &frameInfo.globalDescriptorSet, 0, nullptr);

		for (auto& entity : mEntities) {
			auto& objectM = SO::GameObject::Manager.GetComponent<Components::BoxCollider>(entity);
			PushConstData push{};
			push.modelMatrix = objectM.mat4();
