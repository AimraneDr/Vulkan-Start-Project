...
namespace Systems {
	...
	void WireframeRenderSystem::init(std::shared_ptr<SO::RendererDevice>& device, VkRenderPass renderPass, VkDescriptorSetLayout descriptorSetLayout)
	{
		rDevice = device;
	...
	void WireframeRenderSystem::shutDown() {
		vkDestroyPipelineLayout(rDevice->device(), pipelineLayout, nullptr);
	}


	void WireframeRenderSystem::createPipelineLayout(VkDescriptorSetLayout descriptorSetLayout) {
	...
	void WireframeRenderSystem::createPipeline(VkRenderPass renderPass) {
		...
		gPipeline = std::make_unique<SO::Pipeline>(*rDevice, "C:/VStudio/StartOver/StartOver/shaders/out/wireframe.vert.spv", "C:/VStudio/StartOver/StartOver/shaders/out/wireframe.frag.spv", configInfo);
	}

	void WireframeRenderSystem::bind(VkCommandBuffer commandBuffer, SO::Model& m) {
	...
	void WireframeRenderSystem::draw(VkCommandBuffer commandBuffer, SO::Model& m) {
		if (m.hasIndexBuffer) {
	...
	void WireframeRenderSystem::render(SO::FrameInfo& frameInfo) {
