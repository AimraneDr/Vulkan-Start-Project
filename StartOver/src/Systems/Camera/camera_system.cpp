...
namespace Systems {
	...
	void CameraSystem::update(float dt, GLFWwindow* window) {
		currentCameraId = *mEntities.begin();
		glm::vec3 rotation{ 0 };
		glm::vec3 position{ 0 };

		auto& objTransfomr = SO::GameObject::Manager.GetComponent<Components::TransformComponent>(currentCameraId);
		auto& camera = SO::GameObject::Manager.GetComponent<Components::CameraComponent>(currentCameraId);
		//rotaion actions
